  ...
  return (
    <div className={`space-y-6 ${isMobile ? 'px-4 py-4' : ''}`}>
      <div className="flex items-center justify-between">
        <h1 className={`font-bold tracking-tight ${isMobile ? 'text-2xl' : 'text-3xl'}`}>Task Archiviate</h1>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className={`font-semibold ${isMobile ? 'text-xl' : 'text-2xl'}`}>Task Archiviate</h2>
          <Badge variant="secondary" className={isMobile ? 'text-xs' : ''}>{archivedTasks.length} tasks</Badge>
        </div>

        {isLoading ? (
          <div className="text-center py-8">Caricamento tasks archiviate...</div>
        ) : archivedTasks.length === 0 ? (
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-muted-foreground mb-4">Nessuna task archiviata</p>
            </CardContent>
          </Card>
        ) : (
